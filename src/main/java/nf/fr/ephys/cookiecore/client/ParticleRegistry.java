...
public class ParticleRegistry {
	...
	/**
	 * Registers a particle handler and returns it's particle id
	 * You can register the same handler multiple times and handle the particle type
	 * usung it's id
	 *
	 ...
	 */
	public static EntityFX spawnParticle(int id, double x, double y, double z, double velX, double velY, double velZ) {
		if (id >= particleHandlers.size()) return null;
		...
		if (handler.getMaxParticleSetting(id) > particleSetting) return null;
		...
		if (distanceX * distanceX + distanceY * distanceY + distanceZ * distanceZ > renderDistance * renderDistance) return null;
